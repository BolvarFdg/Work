...
def create_float_mask(
    ...
    """Create a float attention mask from a ``mask_mod`` via direct broadcasting.

    This replaces ``flex_attention.create_mask`` (which internally uses
    ``torch.vmap``) with a pure broadcasting implementation so it works on
    backends like Ascend NPU where ``torch.vmap`` is not supported.
    ...
    if H is None:
        H = 1
    b = torch.arange(0, B, device=device).view(B, 1, 1, 1)
    h = torch.arange(0, H, device=device).view(1, H, 1, 1)
    q = torch.arange(0, Q_LEN, device=device).view(1, 1, Q_LEN, 1)
    kv = torch.arange(0, KV_LEN, device=device).view(1, 1, 1, KV_LEN)

    bool_mask = mask_mod(b, h, q, kv)
